...
2. 첫번째 : 일정량의 유효토큰을 발급해야 하며 나머지는 대기토큰으로 발급한다.
3. 두번째 : 사용자의 포인트 충전은 순차적으로 진행되어야 한다.
4. 세번째 : 좌석 예약에 대한 결제 처리 중 포인트 차감은 일관화 되게 진행되어야 한다.
5. 또한 typeorm의 특성상 lock의 종류를 여러가지 택할 수가 있으며, db의 격리 수준과 lock을 상황에 맞추어 적절하게 사용해야할 것입니다.
